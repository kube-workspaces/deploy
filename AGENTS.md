# AGENTS.md

## Repository: kube-workspaces/deploy

Deployment manifests, documentation, and operational tooling for the kube-workspaces platform.

## Related Repositories

| Repository | Description | Docker Image |
|-----------|-------------|--------------|
| [kube-workspaces/controller](https://github.com/kube-workspaces/controller) | Kubernetes controller (kubebuilder) | `ghcr.io/kube-workspaces/controller` |
| [kube-workspaces/api](https://github.com/kube-workspaces/api) | REST API service (Goa v3) | `ghcr.io/kube-workspaces/api` |
| [kube-workspaces/proxy](https://github.com/kube-workspaces/proxy) | Workspace reverse proxy | `ghcr.io/kube-workspaces/proxy` |
| [kube-workspaces/frontend](https://github.com/kube-workspaces/frontend) | Next.js frontend | `ghcr.io/kube-workspaces/frontend` |
| [kube-workspaces/image-catalog](https://github.com/kube-workspaces/image-catalog) | Source of truth for `Image` CR manifests | n/a |

## Structure

| Directory | Purpose |
|-----------|---------|
| `helm/kube-workspaces/` | Helm chart (includes all components + optional Dex) |
| `kustomize/base/` | Kustomize base deployment |
| `kustomize/crds/` | CRD manifests (synced from controller repo) |
| `kustomize/overlays/auth/` | Auth-enabled overlay |
| `argocd/` | ArgoCD Application manifests |
| `docs/` | Platform documentation |
| `scripts/` | Operational scripts |
| `images.yaml` | Default Image CRs (workspace images) |
| `poddefaults.yaml` | Default PodDefault CRs |

## Commands

```
make install-crd             # install CRDs to current cluster (server-side apply)
make install-images          # apply Image CRs from images.yaml
make sync-images             # vendor images.yaml + helm files/ from image-catalog@IMAGE_CATALOG_VERSION
make check-images            # verify vendored images match IMAGE_CATALOG_VERSION
make deploy-kustomize        # deploy via Kustomize (base)
make deploy-crds             # deploy CRDs via Kustomize
make helm-install            # Helm install
make helm-upgrade            # Helm upgrade
make helm-template           # Helm template (dry-run)
make lint-helm               # Lint Helm chart
make auth-enable             # enable authentication
make auth-disable            # disable authentication
make port-forward-frontend   # port-forward frontend to localhost:3000
make port-forward-api        # port-forward API to localhost:8888
make port-forward-proxy      # port-forward proxy to localhost:8891
```

## Deployment Options

All target `kube-workspaces-system` namespace.

- **Kustomize:** `kubectl apply --server-side -k kustomize/base/`
- **Helm:** `helm install kube-workspaces helm/kube-workspaces/ --namespace kube-workspaces-system --create-namespace`
- **ArgoCD:** `kubectl apply -f argocd/application.yaml`

## Key Notes

- CRD is too large for client-side apply. Always use `kubectl apply --server-side` or `ServerSideApply=true`.
- Images are defined as `Image` CRs (cluster-scoped). Default images in `images.yaml`, vendored from `kube-workspaces/image-catalog` — do not hand-edit, see Image Catalog Sync below.
- CRD YAML files in `kustomize/crds/` are synced from the controller repo's `config/crd/bases/`.
- Docker images: `ghcr.io/kube-workspaces/{controller,api,proxy,frontend}` on GitHub Container Registry.
- Image defaults (defaultEnv, defaultArgs, etc.) are applied at workspace creation time only.
- `defaultEnv` supports `{{namespace}}` and `{{name}}` placeholders.
- Auth is opt-in (disabled by default). Use Helm `auth.enabled: true` or `make auth-enable`.
- PlatformConfig CRD is cluster-scoped singleton named `default`.
- Helm installs a curated 8-image subset by default (`installExampleImages: true`); set `installCatalogImages: true` for the full catalog.
- There are seven `kubeworkspaces.io` CRDs (Workspace, Image, User, AuthConfig, PlatformConfig, PodDefault, SshKey); `kustomize/crds/` must render 7 (see `scripts/validate.sh`).
- KubeVirt is optional and off by default (`kubevirt.enabled: false`). `kubevirt.useEmulation` enables software emulation (no `/dev/kvm`); `kubevirt.cdi.enabled` installs CDI for persistent VM root disks; `kubevirt.gpuPassthrough.permittedHostDevices` allowlists GPU/PCI devices for VM passthrough.
- ArgoCD latest-image workflow: the ArgoCD app pins component image `tag: latest` (rides main pushes live) but pins the chart to a `targetRevision` — chart-side changes (e.g. `kubevirt.gpuPassthrough`) only apply after the coordinated `targetRevision` bump.

## CRD Sync

CRD YAML is generated by the controller repo. When CRDs change:
1. In the controller repo: `make manifests`
2. Copy `config/crd/bases/kubeworkspaces.io_*.yaml` to this repo's `kustomize/crds/`
3. Commit and push

## Image Catalog Sync

`Image` CR manifests are owned by `kube-workspaces/image-catalog`, not this repo. To pick up a new release:
1. Bump `IMAGE_CATALOG_VERSION` in `Makefile`
2. Run `make sync-images`
3. Commit `images.yaml` and `helm/kube-workspaces/files/images-*.yaml`

## Docs API Reference Sync

`docs/api.md` renders ReDoc of the REST API, and `docs/openapi3.{json,yaml}`
are vendored from the api repo's goa output (owned by `kube-workspaces/api`); the
website build stages them under `/assets/docs/`. When the API changes
(`api/design/design.go`):
1. In the api repo: `go run goa.design/goa/v3/cmd/goa gen github.com/kube-workspaces/api/design`
2. Copy `api/gen/http/openapi3.json` and `api/gen/http/openapi3.yaml` here as `docs/openapi3.json` and `docs/openapi3.yaml`
3. Commit and push (the `docs-notify` workflow triggers the website rebuild)
